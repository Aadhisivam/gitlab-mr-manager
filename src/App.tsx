...
import { setGitLabToken, fetchProjects, fetchMergeRequests, mergeMR, fetchLatestPipeline, setGitLabUrl, fetchProjectLabels } from './services/gitlab';
  ...
  const handleMergeSelected = async () => {
    ...
    const promises = selectedMRs.map(async (mrId) => {
      ...
      try {
        ...
        toast.success(`Merged: ${mr.title}`);
      } catch (error) {
        toast.error(`Failed to merge: ${mr.title}`);
      }
    });
          ...
          <div className="space-y-8">
            ...
            {selectedProject && (
              <>
                ...
                <PipelineStatus pipeline={latestPipeline} />
              </>
            )}
          </div>
        )}
